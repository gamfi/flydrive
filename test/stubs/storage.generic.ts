...
type testSpec = {
    ...
    options?: {
        ...
        nondeterministicDelete?: boolean,
    }
}
...
export function runGenericStorageSpec({name, storage, options}: testSpec) {
    ...
    const skipable = (feature: 'getUrl'|'getSignedUrl', func: () => void): void => {
        if (!options?.skip || options.skip.indexOf(feature) === -1) {
            func();
        }
    };
    ...
    describe(name, () => {
        ...
        describe('.delete', () => {
            test('deletes file from storage (set-up with .put, checked with .exists)', async () => {
                ...
                // test
                const { wasDeleted } = await storage.delete(targetPath);

                // check
                expect((await storage.exists(targetPath)).exists).toStrictEqual(false);

                if (options?.nondeterministicDelete) {
                    expect(wasDeleted).toStrictEqual(null);
                } else {
                    expect(wasDeleted).toStrictEqual(true);
                }
            });

            test('returns { wasDeleted: false|undefined } when no file was deleted', async () => {
                // setup
                const targetPath = randomFilePath();
